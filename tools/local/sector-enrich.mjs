...
function loadUniverse() {
  if (!existsSync(LATEST_UNIVERSE)) {
    fail("latest common-stock universe is missing; run npm run local:sync first");
  }
...
function recordPath(asOf, symbol) {
  return path.join(REFERENCE_ROOT, `as-of=${asOf}`, "symbols", `${safeSymbol(symbol)}.json`);
}

function manifestPath(asOf) {
  return path.join(REFERENCE_ROOT, `as-of=${asOf}`, "manifest.json");
}
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt += 1) {
    ...
    try {
      response = await fetch(url, {
        headers: { Accept: "application/json", "User-Agent": "FutureView-Sector-Enrichment/1.0" },
...
function normalizeOverview(symbol, asOf, response) {
  ...
  return {
    ...
    cik: item.cik ?? null,
    sic_code: item.sic_code == null ? null : String(item.sic_code),
    sic_description: item.sic_description ?? null,
    description: item.description ?? null,
    market_cap: item.market_cap ?? null,
    list_date: item.list_date ?? null,
    source: "massive-ticker-overview",
    updated_at: now,
  };
}
...
function summarize(asOf, universeAsOf, symbols) {
  ...
  return {
    ...
    cached_count: cached,
    remaining_count: Math.max(0, symbols.length - cached),
    with_sic_count: withSic,
    sic_coverage_of_cached: cached ? withSic / cached : null,
    status_counts: statusCounts,
    sic_code_counts: sicCounts,
    sample_missing_sic: missingSic,
    source: "massive-ticker-overview",
