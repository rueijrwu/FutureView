(() => {
  ...
  function render(b){candles.update(candle(b));volume.update(vol(b))}function reset(bs){candles.setData(bs.map(candle));volume.setData(bs.map(vol));chart.timeScale().fitContent()}function error(m=""){$("error").textContent=m}
  async function api(path,opts={}){const r=await fetch(`${API_ORIGIN}${path}`,{headers:{"Content-Type":"application/json"},...opts});if(!r.ok){let m=`HTTP ${r.status}`;try{m=(await r.json()).error||m}catch{}throw new Error(m)}return r.json()}
  ...
  function connect(path){if(ws)ws.close();const wsOrigin=API_ORIGIN.replace(/^http/,"ws");ws=new WebSocket(`${wsOrigin}${path}`);ws.onmessage=e=>{const x=JSON.parse(e.data);if(x.type==="bar")render(x.bar);else if(x.type==="bars_batch")x.bars.forEach(render);else if(x.type==="reset"){reset(x.warmup||[]);update(x.snapshot)}else if(x.type==="error")error(x.error);else update(x)};ws.onerror=()=>error("Replay WebSocket disconnected")}
  async function loadContracts(){const x=await api("/api/contracts");$("contract").innerHTML=x.contracts.map(c=>`<option value="${c.contract}">${c.contract}</option>`).join("");if(x.contracts.length)selectContract(x.contracts[0].contract,x.contracts[0])}
  async function selectContract(name,provided){const c=provided||await api(`/api/contracts/${encodeURIComponent(name)}`);$("range").textContent=`${displaySeconds(c.first_time)} → ${displaySeconds(c.last_time)} · ${Number(c.bars).toLocaleString()} bars`;$("start").value=inputValueFromSeconds(c.first_time)}
  $("contract").onchange=()=>selectContract($("contract").value).catch(e=>error(e.message));$("start-btn").onclick=async()=>{try{error();const raw=$("start").value;if(!raw)throw new Error("Choose a start time");const x=await api("/api/replay/sessions",{method:"POST",body:JSON.stringify({contract:$("contract").value,start:wallTimeToUtcIso(raw),warmup:Number($("warmup").value||300)})});sessionId=x.session_id;reset(x.warmup||[]);update(x);connect(x.websocket)}catch(e){error(e.message)}};
  $("play").onclick=()=>command("play",{speed});$("pause").onclick=()=>command("pause");$("next").onclick=()=>command("step");$("restart").onclick=()=>command("restart");$("speeds").onclick=e=>{const b=e.target.closest("button[data-speed]");if(!b)return;document.querySelectorAll("#speeds button").forEach(x=>x.classList.remove("active"));b.classList.add("active");speed=b.dataset.speed==="max"?"max":Number(b.dataset.speed);if(lastState==="PLAYING")command("play",{speed})};loadContracts().catch(e=>error(e.message));update({state:"STOPPED"});
})();
