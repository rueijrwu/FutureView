(() => {
  ...
  function renderTrading(){
    ...
    if(!t){
      $("trade-position").textContent="Flat";$("trade-avg").textContent="—";$("trade-unrealized").textContent="$0.00";$("trade-realized").textContent="$0.00";$("trade-total").textContent="$0.00";$("trade-rows").innerHTML='<tr class="trade-empty"><td colspan="5">No trades yet</td></tr>';$("trades-toggle").textContent="Trades";return;
    }
  ...
  function render(b){
    ...
    if(range){try{chart.timeScale().setVisibleRange(range)}catch{}preserveNextRange=null}
  }
  ...
  function reset(bs){chartTools._cancelDrawing?.();candles.setData(bs.map(candle));volume.setData(bs.map(vol));chartTools.reset(bs);chartTools.fit();lastMarkPrice=bs.length?Number(bs[bs.length-1].c):null;selectedFillId=null;preserveNextRange=null;renderTradeMarkers()}
  ...
  function command(type,extra={}){if(!ws||ws.readyState!==WebSocket.OPEN||!wsSynced){error("Replay socket is not synchronized - reconnecting…");if(wsPath)connect(wsPath);return}pendingCommand=type;if(type==="play")lastState="PLAYING";else if(type==="pause"||type==="restart")lastState="PAUSED";update({state:lastState});ws.send(JSON.stringify({type,...extra}));clearTimeout(commandAckTimer);commandAckTimer=setTimeout(()=>{if(pendingCommand===type){pendingCommand=null;wsSynced=false;preserveNextRange=null;error("Replay command acknowledgement timed out - resynchronizing…");syncControls();if(wsPath)connect(wsPath)}},2000)}
  ...
  $("play").onclick=()=>command("play",{speed});$("pause").onclick=()=>command("pause");$("next").onclick=()=>{preserveNextRange=chart.timeScale().getVisibleRange?.()||null;command("step")};$("restart").onclick=()=>command("restart");
