(() => {
  ...
  async function loadRange(){replayRangeInfo=await api(`/api/replay/range?product=${$("product").value||"MES"}`);$("product").value=replayRangeInfo.product;$("start").value=inputValueFromSeconds(replayRangeInfo.first_time);$("range").textContent=`${displaySeconds(replayRangeInfo.first_time)} → ${displaySeconds(replayRangeInfo.last_time)} · contract selected automatically`}
