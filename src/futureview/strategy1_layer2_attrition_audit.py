...
from .strategy1_layer2_centered_train import _centered_targets, _feature_series
...
def main() -> None:
    ...
    # Exact unit alignments to legal Entry decisions.
    ...
    # Current Layer-2 behavior: each target decision inherits the most recent prior non-neutral gate.
    inherited = []
    ...
    print(
        ...
        f"neutral={sum(g==0 for _,_,g in gate_rows)} low={sum(g==-1 for _,_,g in gate_rows)} "
    ...
    if inherited:
        ...
        print(
            ...
            f"p90={np.quantile(lags,0.90):.1f} max={lags.max()} same_or_next30={int((lags<=W).sum())}"
        )
