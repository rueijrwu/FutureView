...
def purged_expanding_walk_forward(
    ...
    """Create complete expanding chronological OOS folds separated by a purge gap."""
    if n_samples <= 0:
        raise ValueError("n_samples must be positive")
    ...
    if not folds:
        raise ValueError(
            f"no complete walk-forward folds: n={n_samples} min_train={min_train} purge={purge} test={test_size}"
        )
