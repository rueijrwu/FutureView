...
from .strategy1_cq_data import HORIZON, LOOKBACK, SCALES, make_cq_labels, make_input_windows
...
def main() -> None:
    ...
    print(
        f"S1 SMH_CQ_RANK DATA ticker={TICKER} period={DATA_PERIOD} rows={audit.rows} "
        f"start={audit.start} end={audit.end} history_entries={len(mu)} folds={len(folds)} "
        f"lookback={LOOKBACK} horizon={HORIZON} live_holdout_start={pd.Timestamp(live_start).date()} "
        f"live_holdout_end={pd.Timestamp(live_end).date()}"
    )
    print(
        ...
        "no_open=true no_high=true no_low=true no_return=true no_ma_feature=true "
        "no_rsi=true no_macd=true no_atr=true"
    )
    print(
        "S1 SMH_CQ_RANK LABELS source=strategy1_cq_data values=L,mu,U,C,Q "
        "C=U-L Q=(U-mu)/(U-L) labels_not_input=true"
    )
    print(
        "S1 SMH_CQ_RANK TARGET stage1=mu_gate stage2=pairwise_preference "
        "preference_rule=C_higher_and_Q_lower no_scalar_composite=true "
        "no_L_mu_U_regression=true"
    )
    print(
        f"S1 SMH_CQ_RANK SPLIT type=expanding_chronological_purged purge_days={PURGE_DAYS} "
        f"min_train={MIN_TRAIN_SAMPLES} min_test={MIN_TEST_SAMPLES} legacy_training_dependency=false"
    )
    ...
    for fold_id, (train, test) in enumerate(folds, start=1):
        ...
        for seed in SEEDS:
            ...
            print(
                f"S1 SMH_CQ_RANK FOLD_RESULT id={fold_id} seed={seed} pairs={total} "
                f"correct={correct} pair_accuracy={acc:.6f}"
            )
